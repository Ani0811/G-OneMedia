...
export default function ClientProjects() {
  ...
  // Determine dynamic container class: 1 card -> centered large preview (max-w-3xl lg:max-w-4xl), 2 cards -> 2-col max-w-5xl, 3+ cards -> 3-col max-w-7xl
  const getContainerLayoutClass = (count) => {
    if (count === 1) return 'flex justify-center max-w-3xl lg:max-w-4xl mx-auto w-full'
    if (count === 2) return 'grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto w-full'
            ...
            {displayedProjects.map((project, idx) => {
              ...
              return (
                <motion.a
                  ...
                  className={`group block rounded-3xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/30 hover:bg-white/[0.04] transition-all duration-300 relative flex flex-col justify-between h-full shadow-2xl hover:shadow-[0_12px_40px_rgba(0,240,255,0.12)] ${
                    isSingle ? 'w-full max-w-3xl lg:max-w-4xl p-6 sm:p-8 md:p-10' : 'w-full p-6'
                  }`}
                >
                  <div className="flex flex-col grow">
                    {/* Visual Image container */}
                    <div className={`relative rounded-2xl overflow-hidden mb-6 border border-white/10 bg-black/50 shadow-inner ${
                      isSingle ? 'aspect-video md:aspect-[16/10]' : 'aspect-video'
                    }`}>
                      <img
                        src={resolvedImage}
                        alt={project.title}
                        className="w-full h-full object-cover object-top group-hover:scale-103 transition-transform duration-700 opacity-95"
                        loading="lazy"
                      />
                      {project.live_url && (
                    ...
                    <div className="space-y-3 grow flex flex-col">
                      <div className="flex items-center justify-between">
                        <span className={`inline-block font-black uppercase tracking-wider text-cyan-400 ${
                          isSingle ? 'text-xs' : 'text-[9px]'
                        }`}>
                          {project.category || 'Web Application'}
                        </span>
                        {project.client_name && (
                          <span className={`text-[var(--text-muted)] font-semibold ${
                            isSingle ? 'text-xs md:text-sm' : 'text-[10px]'
                          }`}>
                            {project.client_name}
                          </span>
                        )}
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <h3 className={`font-bold text-white group-hover:text-cyan-400 transition-colors ${
                          isSingle ? 'text-2xl md:text-3xl tracking-tight' : 'text-lg'
                        }`}>
                          {project.title}
                        </h3>
                        {project.live_url && (
                          <ExternalLink size={isSingle ? 18 : 14} className="text-[var(--text-muted)] group-hover:text-cyan-400 transition-colors shrink-0" />
                        )}
                      </div>

                      <p className={`text-[var(--text-muted)] leading-relaxed grow ${
                        isSingle ? 'text-sm md:text-base' : 'text-xs line-clamp-3'
                      }`}>
                        {project.description}
                      </p>
                    </div>
                  ...
                  {/* Tech Stack tags */}
                  {techList.length > 0 && (
                    <div className={`flex flex-wrap gap-2 border-t border-white/5 ${
                      isSingle ? 'pt-5 mt-6' : 'pt-4 mt-4'
                    }`}>
                      {techList.map((t, i) => (
                        <span
                          key={i}
                          className={`rounded-lg bg-white/5 text-[var(--text-secondary)] border border-white/5 font-medium ${
                            isSingle ? 'text-xs px-3 py-1' : 'text-[9px] px-2 py-0.5'
                          }`}
                        >
                          {t}
                        </span>
